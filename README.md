...
| File | Description |
|---|---|
| `PigNet.py` | Proposed PigNet model with an LSTM encoder, FiLM conditioning, channel recalibration, horizon-query attention, and depthwise-separable horizon smoothing. |
| `TimesNet.py` | TimesNet forecasting baseline. |
| `LSTM-10_fold_CV_win14.py` | Standard LSTM forecasting baseline. |
| `LightTS.py` | LightTS/IEBlock forecasting baseline. |
| `XGBoost-10_fold_CV_win14.py` | XGBoost baseline with exhaustive inner hyperparameter search. |
| `Light-10_fold_CV_win14.py` | LightGBM baseline with exhaustive inner hyperparameter search. |
| `RF-10_fold_CV_win14.py` | Random Forest baseline with exhaustive inner hyperparameter search. |
| `Cat-10_fold_CV_win14.py` | CatBoost baseline with exhaustive inner hyperparameter search. |
| `simple_baselines.py` | Linear and quadratic limited-input growth baselines. |

### Shared protocol and verification scripts

| File | Description |
|---|---|
| `pignet_common.py` | Shared data loading, preprocessing, sliding-window construction, canonical pig-level fold assignment, scaling, evaluation, and Excel reporting utilities. |
| `deep_model_runner.py` | Leakage-free nested cross-validation runner for PigNet, TimesNet, LSTM, and LightTS. |
| `tree_model_runner.py` | Leakage-free nested cross-validation and exhaustive hyperparameter-search runner for the four tree-based models. |
| `verify_revised_code.py` | Static and synthetic checks for syntax, shared fold usage, grid sizes, nested partitioning, neural output shapes, and simplified nested-CV execution. |
...
1. only the outer-training pigs enter inner model selection;
2. deterministic inner 5-fold pig-level validation is created;
3. each inner fold trains for at most 400 epochs;
4. pig-level macro-averaged RMSE controls learning-rate reduction and early stopping;
5. the rounded median of the five best epochs is selected;
6. the model is reinitialized and retrained from scratch on all outer-training pigs;
7. the outer-validation pigs are evaluated only after final training.
...
1. exhaustive Cartesian-grid search is performed using only the outer-training pigs;
2. candidate configurations are ranked by mean inner-fold pig-level macro RMSE;
3. the selected configuration is refitted on all outer-training pigs;
4. the outer-validation pigs are used once for final evaluation.
