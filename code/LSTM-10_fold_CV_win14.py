...
新增/重点输出：
- MacroOverall_Val：逐猪 RMSE 宏平均（Val） 的 mean/std/95%CI/CV
- MacroHorizon_Val：逐猪 RMSE 宏平均（Val, h1..h7 及 h_avg） 的 mean/std/95%CI/CV
...
OUT_EXCEL = r"E:\文献\课题\AI\猪场实验数据\正式试验结果\最终结果\win14\LSTM_10_fold_CV_win14"  # 将生成 _W7_H7_Outer10CV.xlsx
...
USE_HUBER = True
USE_LR_SCHEDULER = True      # 你的 ReduceLROnPlateau 可保留
EARLY_STOPPING = True
EARLY_PATIENCE = 25
...
# ---------- LSTM 模型 ----------
class LSTM2MultiH(nn.Module):
    def __init__(self, in_dim: int, out_h: int, hidden_size=96, num_layers=2, dropout=0.2):
        super().__init__()
        self.lstm = nn.LSTM(input_size=in_dim, hidden_size=hidden_size, num_layers=num_layers,
                            dropout=dropout if num_layers > 1 else 0.0, batch_first=True)
        self.head = nn.Sequential(
            nn.Linear(hidden_size, hidden_size // 2),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_size // 2, out_h)
        )
    def forward(self, x):  # x: (B, T, F)
        out, _ = self.lstm(x)         # (B, T, H)
        last = out[:, -1, :]          # (B, H)
        return self.head(last)        # (B, out_h)
...
def train_one_model(Xtr, Ytr, Xva=None, Yva=None, params: dict | None = None, device: str | None = None):
    ...
    model = LSTM2MultiH(in_dim=Xtr.shape[-1], out_h=Ytr.shape[-1],
...
def model_predict(model: LSTM2MultiH, X: np.ndarray, device: str | None = None) -> np.ndarray:
...
def eval_by_pig_named(samples_list: List[dict], yhat_list: List[np.ndarray], set_name: str, horizon: int) -> pd.DataFrame:
    """
    逐猪评估（等权口径）：
    - 先算该猪在每个地平线 h 的 RMSE_h（对该猪该 h 的所有窗口做 RMSE）
    - 再做 7 天算术平均：RMSE = mean(RMSE_h1..RMSE_h{H})
    - R2 也做 7 天等权平均（跳过 NaN）
    - y_std 为 7 天 std 的等权平均；rmse_over_std = RMSE / y_std
    注意：n_win / n 仅记录，不参与任何加权。
    """
    rows = []
...
def bootstrap_mean_ci(values: np.ndarray, B: int = 2000, alpha: float = 0.05, seed: int = SEED):
    """对一维数列做 bootstrap 均值置信区间；返回 (mean, std, lo, hi, cv)。"""
...
def macro_over_pigs_horizon(df_val_pig: pd.DataFrame, H: int):
    """Val逐猪各地平线 RMSE + h_avg → 宏平均统计（每行一个水平）。"""
    rows = []
    # 各地平线
    for h in range(1, H+1):
        ...
        if col in df_val_pig.columns:
            ...
            rows.append(dict(level=f"h{h}", mean=mean, std=std, CI95_lo=lo, CI95_hi=hi,
                             CI_width=(hi-lo) if (not math.isnan(hi) and not math.isnan(lo)) else np.nan,
                             CV=cv, n_pigs=int(len(vals))))
    # h_avg：先对每头猪取 h1..h7 的均值，再对猪做宏平均
...
def stability_over_folds(fold_macro_means: List[float]):
    """逐折 Val 宏平均 RMSE 列表 → 折间统计（mean/std/CI/CV）。"""
...
def run_outer10cv_eval_fixed_params():
    ...
    print(f"✅ 外层按猪 {n_splits} 折（固定超参）评估完成：{out_file}")

# ========== 构建 X/Y/Groups（依赖的函数放在后面，便于阅读） ==========
def build_Xy_groups(samples_list: List[dict]):
    X_parts, Y_parts, groups = [], [], []
    for s in samples_list:
        X_seq = s["X"]
