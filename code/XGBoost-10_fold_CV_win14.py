# -*- coding: utf-8 -*-
"""
XGBoost（天粒度，固定滑窗 W=7 → H=7，多输出 MIMO）
...
依赖：
  pip install xgboost scikit-learn pandas numpy xlsxwriter shap
"""
...
OUT_EXCEL = r"E:\文献\课题\AI\猪场实验数据\正式试验结果\最终结果\win14\XGB_10_fold_CV"  # 生成 _W7_H7.xlsx
...
# ======== XGBoost 基础与候选（5折调参用） ========
USE_GPU = True  # 如有 NVIDIA GPU 且 xgboost 支持，可设 True（会加 tree_method="gpu_hist"）
...
PARAM_CANDIDATES: List[dict] = [
    dict(n_estimators=800,  learning_rate=0.05, max_depth=4, subsample=0.9, colsample_bytree=0.9, min_child_weight=1, reg_lambda=1.0),
    dict(n_estimators=1000, learning_rate=0.05, max_depth=6, subsample=0.8, colsample_bytree=0.8, min_child_weight=1, reg_lambda=1.0),
    dict(n_estimators=1200, learning_rate=0.03, max_depth=6, subsample=0.9, colsample_bytree=0.9, min_child_weight=1, reg_lambda=1.0),
    dict(n_estimators=800,  learning_rate=0.05, max_depth=8, subsample=0.8, colsample_bytree=0.8, min_child_weight=3, reg_lambda=1.5),
]
INNER_FOLDS_TUNE = 5
OUTER_FOLDS_EVAL = 10
SEED = 42
...
def _macro_rmse_over_pigs_equal_weight(Y_val: pd.DataFrame, Yhat_val: np.ndarray, groups_val: np.ndarray, horizon: int) -> float:
    """对每头猪：先算 h1..h{H} 的 RMSE，然后对 7 天做均值；最后在猪之间做简单平均。"""
...
def tune_params_global_5fold(X: pd.DataFrame, Y: pd.DataFrame, groups: np.ndarray,
    ...
    for cand in (candidates or [dict()]):
        ...
        if USE_GPU:
            params = dict(params); params.update(device="cuda")  # 可选 GPU
        scores = []
        ...
        logs.append(dict(inner_folds=n_splits, mean_macro_RMSE=mean_score, **cand))
...
def add_set_means_and_agg(writer, df_per_pig: pd.DataFrame, base_sheet_name: str, horizon: int,
    ...
    """
    写某集合（默认 Val）的：
      1) 每一天（h1..h7）逐猪均值（RMSE、R²）
      2) 7天逐猪平均（RMSE 与 R² 各一组）的 mean/median/std
    """
    if df_per_pig.empty: return
    df = df_per_pig[df_per_pig["集合"] == target_set].copy()
    if df.empty: return
...
def compute_shap_for_multioutput(model, X_in: pd.DataFrame, groups: np.ndarray | None,
    ...
    """
    对 MultiOutputRegressor 内部的 7 个 XGB 子模型分别做 SHAP；
    返回：
    ...
    for h in range(horizon):
        est_h = model.estimators_[h]  # 第 h 个 XGBRegressor
        explainer = shap.TreeExplainer(est_h)
...
def build_global_means(cv_all: pd.DataFrame, horizon: int) -> pd.DataFrame:
    """
    返回一个表：对 Train/Val 分别给出：
      - N_rows（行数）/ N_pigs（去重猪数）
      - RMSE_mean / R2_mean
      - RMSE_h1_mean..RMSE_h7_mean / R2_h1_mean..R2_h7_mean
    """
    rows = []
...
def run_tune5cv_then_eval10cv():
    ...
    best_params, tuning_logs = tune_params_global_5fold(
        ...
        base_params=BASE_XGB_PARAMS, candidates=PARAM_CANDIDATES,
        horizon=HORIZON, inner_folds=INNER_FOLDS_TUNE
    )
    if USE_GPU:
        best_params = dict(best_params); best_params.update(device="cuda")
    ...
    with pd.ExcelWriter(out_file, engine="xlsxwriter") as writer:
        ...
        if all_val_rows:
            ...
            # （新增）所有的均值（总的）：Train/Val 的整体均值（不带CI）
            global_means = build_global_means(pd.concat(all_val_rows + [], ignore_index=True), HORIZON)
            # 注意：all_val_rows 里只有 Val，如果你也想同时统计 Train，可在循环中另存 all_train_rows 再 concat 传入。
            # 这里按你的当前口径，仅对 Val 进行“总的”均值汇总。
    ...
    print(f"✅ 已完成：5折调参（按猪）→ 10折评估（按猪）。结果写入：{derive_out_path(OUT_EXCEL, WINDOW, HORIZON)}")
