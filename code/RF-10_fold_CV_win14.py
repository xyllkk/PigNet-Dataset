# -*- coding: utf-8 -*-
"""
RandomForest（天粒度，固定滑窗 W=7 → H=7，多输出 MIMO）
...
依赖：
  pip install scikit-learn pandas numpy xlsxwriter shap
"""
...
OUT_EXCEL = r"E:\文献\课题\AI\猪场实验数据\正式试验结果\最终结果\win14\RF_10_fold_CV_win14"  # 生成 _W7_H7.xlsx（沿用原命名）
...
BASE_RF_PARAMS = dict(
    ...
    max_samples=0.8,         # ~= subsample（需 bootstrap=True 才生效）
    min_samples_leaf=1,
    bootstrap=True,
    oob_score=False,
    n_jobs=-1,
...
# 将你原 XGBoost 的 4 组候选网格做“语义映射”到 RF：
PARAM_CANDIDATES: List[dict] = [
    dict(n_estimators=800,  max_depth=4, max_features=0.9, max_samples=0.9, min_samples_leaf=1),
    dict(n_estimators=1000, max_depth=6, max_features=0.8, max_samples=0.8, min_samples_leaf=1),
    dict(n_estimators=1200, max_depth=6, max_features=0.9, max_samples=0.9, min_samples_leaf=1),
    dict(n_estimators=800,  max_depth=8, max_features=0.8, max_samples=0.8, min_samples_leaf=3),
]
INNER_FOLDS_TUNE = 5
OUTER_FOLDS_EVAL = 10
SEED = 42
...
def compute_shap_for_multioutput(model, X_in: pd.DataFrame, groups: np.ndarray | None,
    ...
    """
    对 MultiOutputRegressor 内部的 7 个 RF 子模型分别做 SHAP；
    返回：
    ...
    for h in range(horizon):
        est_h = model.estimators_[h]  # 第 h 个 RandomForestRegressor
...
def run_tune5cv_then_eval10cv():
    ...
    best_params, tuning_logs = tune_params_global_5fold(
        ...
        base_params=BASE_RF_PARAMS, candidates=PARAM_CANDIDATES,
    ...
    with pd.ExcelWriter(out_file, engine="xlsxwriter") as writer:
        for fold_idx, (tr_idx, va_idx) in enumerate(gkf_outer.split(X_all, Y_all, groups_all), start=1):
            ...
            est_final = RandomForestRegressor(**best_params)
    ...
    print(f"✅ 已完成：RandomForest 5折调参（按猪）→ 10折评估（按猪）。结果写入：{derive_out_path(OUT_EXCEL, WINDOW, HORIZON)}")
