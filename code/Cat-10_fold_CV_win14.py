# -*- coding: utf-8 -*-
"""
CatBoost（天粒度，固定滑窗 W=7 → H=7，多输出 MIMO）
...
依赖：
  pip install catboost scikit-learn pandas numpy xlsxwriter shap
"""
...
import shap  # 用于树模型 SHAP 解释

# =========================
# 手动参数区（只改这里）
# =========================
DATA_PATH = r"E:\文献\课题\AI\猪场实验数据\输入_采食加环境加日龄_按天"
OUT_EXCEL = r"E:\文献\课题\AI\猪场实验数据\正式试验结果\最终结果\win14\Cat_10_fold_CV_win14"  # 生成 _W7_H7.xlsx（保持原文件名习惯）
...
# ======== CatBoost 基础与候选（5折调参用） ========
USE_GPU = False  # 如有 NVIDIA GPU 且 CatBoost 支持，可设 True（会用 task_type="GPU"）
BASE_CAT_PARAMS = dict(
    loss_function="RMSE",
    random_seed=42,
    iterations=1000, learning_rate=0.05, depth=6,
    subsample=0.8, rsm=0.8, l2_leaf_reg=1.0, min_data_in_leaf=1,
    bootstrap_type="Bernoulli",   # ← 新增：使 subsample 生效
    thread_count=-1, verbose=False
)
# 下面把你原来 XGBoost 的候选手动“等价”映射到 CatBoost 参数域
PARAM_CANDIDATES: List[dict] = [
    dict(iterations=800,  learning_rate=0.05, depth=4, subsample=0.9, rsm=0.9, min_data_in_leaf=1, l2_leaf_reg=1.0),
    dict(iterations=1000, learning_rate=0.05, depth=6, subsample=0.8, rsm=0.8, min_data_in_leaf=1, l2_leaf_reg=1.0),
    dict(iterations=1200, learning_rate=0.03, depth=6, subsample=0.9, rsm=0.9, min_data_in_leaf=1, l2_leaf_reg=1.0),
    dict(iterations=800,  learning_rate=0.05, depth=8, subsample=0.8, rsm=0.8, min_data_in_leaf=3, l2_leaf_reg=1.5),
]
INNER_FOLDS_TUNE = 5
OUTER_FOLDS_EVAL = 10
SEED = 42
...
def tune_params_global_5fold(X: pd.DataFrame, Y: pd.DataFrame, groups: np.ndarray,
    ...
    for cand in (candidates or [dict()]):
        ...
        # GPU/CPU 切换
        params = dict(params, task_type=("GPU" if USE_GPU else "CPU"))
        scores = []
...
def compute_shap_for_multioutput(model, X_in: pd.DataFrame, groups: np.ndarray | None,
    ...
    """
    对 MultiOutputRegressor 内部的 7 个 CatBoost 子模型分别做 SHAP；
    返回：
    ...
    for h in range(horizon):
        est_h = model.estimators_[h]  # 第 h 个 CatBoostRegressor
...
def run_tune5cv_then_eval10cv():
    ...
    best_params, tuning_logs = tune_params_global_5fold(
        ...
        base_params=BASE_CAT_PARAMS, candidates=PARAM_CANDIDATES,
    ...
    with pd.ExcelWriter(out_file, engine="xlsxwriter") as writer:
        for fold_idx, (tr_idx, va_idx) in enumerate(gkf_outer.split(X_all, Y_all, groups_all), start=1):
            ...
            est_final = CatBoostRegressor(**best_params)
    ...
    print(f"✅ 已完成：CatBoost 5折调参（按猪）→ 10折评估（按猪）。结果写入：{derive_out_path(OUT_EXCEL, WINDOW, HORIZON)}")
